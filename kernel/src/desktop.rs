...
impl Desktop {
    ...
    /// Changes the wallpaper at runtime. `path` is a name like
    /// "wallpaper:nuit" or "wallpaper:ocean"; unknown names fall back to the
    /// default. The desktop redraws itself at once.
    pub fn set_wallpaper(&mut self, path: &str) {
        let name = path.strip_prefix("wallpaper:").unwrap_or(path);
        let index = WALLPAPERS.iter().position(|(key, _)| *key == name).unwrap_or(0);
