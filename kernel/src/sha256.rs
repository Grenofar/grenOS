...
pub fn pbkdf2(password: &[u8], salt: &[u8], iterations: u32, out: &mut [u8]) {
    ...
    while done < out.len() {
        ...
        // U1 = PRF(P, S || INT(i)): the salt first, then the block index
        // (RFC 2898 §5.2). The first version had them the other way round, and
        // its self-test said so on the serial line while CI stayed green.
